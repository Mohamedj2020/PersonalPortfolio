...
const Contact = () => {
  ...
  // Now useForm will always have a valid ID
  const [state, handleSubmit] = useForm(formId);

  // Use a conditional for rendering
  return (
    <section id="contact" className="py-20">
      <div className="container mx-auto px-6">
        ...
        {!formId ? (
          // Show this when no form ID is available
          <div className="text-center p-6 bg-red-50 text-red-800 rounded max-w-2xl mx-auto">
            <p>Contact form unavailable - environment variable missing</p>
            <p className="text-sm mt-2">Add REACT_APP_FORMSPREE_ID to .env file</p>
          </div>
        ) : (
          // Show the actual form when ID is available
          <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-12">
            ...
            <div>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <ValidationError errors={state.errors} className="text-red-400 mt-2" />
              </form>
            </div>
          </div>
        )}
