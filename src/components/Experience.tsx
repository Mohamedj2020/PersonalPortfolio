...
const Experience = () => {
  ...
  const experiences = [
    ...
    },
    {
      ...
      technologies: ["C++", "CUDA", "GPU Computing", "Compiler Design"]
    },
    {
      ...
      technologies: ["C", "Teaching", "Curriculum Development", "Mentorship"]
    }
  ];
          ...
          <div className="mb-16">
            ...
            <div className="relative">
              {/* Timeline line */}
              <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-teal-400 via-cyan-400 to-blue-400 hidden md:block"></div>
              
              <div className="space-y-8">
                {experiences.map((exp, index) => (
                  <div key={index} className="relative flex items-start animate-on-scroll">
                    {/* Timeline dot */}
                    <div className="absolute left-6 w-4 h-4 bg-teal-400 rounded-full border-4 border-gray-900 z-10 hidden md:block"></div>
                    
                    {/* Experience card */}
                    <div className="md:ml-16 w-full">
                      <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-teal-400/30 transition-all duration-300 h-full">
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                          <div className="flex-1">
                            <h4 className="text-xl font-bold text-white mb-2">{exp.title}</h4>
                            <p className="text-teal-300 font-semibold text-lg mb-1">{exp.organization}</p>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {exp.technologies.map((tech, techIndex) => (
                            <span key={techIndex} className="px-3 py-1 bg-teal-600/20 text-teal-300 rounded-full text-sm border border-teal-600/30">
                              {tech}
                            </span>
                          ))}
                        </div>
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
              {Object.entries(skills).map(([category, skillList], categoryIndex) => (
                <div key={category} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 animate-on-scroll">
